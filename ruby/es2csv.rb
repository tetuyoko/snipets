...
def request_uri
  URI.parse("http://#{config[:host]}/")
end

# Elasticsearch へのリクエスト
def get_respons(request)
  begin
    Net::HTTP.start(request_uri.host, :open_timeout => 10) do |http|
...
def check_document_field_name
  ...
  begin
    ...
    puts json.values[0]["mappings"][config[:type_prefix]]["properties"].keys
  rescue => ex
    puts ex.message
  end
end
...
def search_document(*params)
  ...
  request = Net::HTTP::Post.new(req, {'Content-Type' =>'application/json'})
  begin
...
if params["c"]
  puts "ドキュメント数:" 
  check_max_document_count
  puts "\n"
  puts "ドキュメントフィールド一覧:"
  check_document_field_name
  puts "\n"
else
